...
NETS = {'ResNet-101': ('ResNet-101',
		  'chimney_SV_resnet101_rfcn_40000.caffemodel'),
  ...
  if os.path.isfile(im_file):
   #   try:
    ...
    # Visualize detections for each class
          CONF_THRESH = 0.8
          NMS_THRESH = 0.3
	  outputPath = os.path.join(img_path, 'output')
	  if not os.path.exists(outputPath):
          ...
          for i in inds:
            bbox = dets[i, :4]
            score = dets[i, -1]
            print("%s: %f %f %f %f; %f" %(image_name, dets[i, 0], dets[i, 1], dets[i, 2], dets[i, 3], dets[i, -1]))
...
def parse_args():
    ...
    parser.add_argument('--net', dest='demo_net', help='Network to use [ResNet-101]',
                        default='ResNet-101')
    parser.add_argument('--model', dest='demo_model', help='Model to use [ResNet-101]',
                        default='ResNet-101')
